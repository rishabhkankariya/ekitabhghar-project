...
<body>
    ...
    <form id="addSyllabusForm" enctype="multipart/form-data">
    ...
    <script>
        $(document).ready(function () {
            ...
            function fetchYearsSemesters() {
                $.post("manage_syllabus.php", { action: "fetch_years_semesters" }, function (response) {
            ...
            function fetchSyllabus() {
                $.post("manage_syllabus.php", { action: "fetch" }, function (response) {
                    if (response.status === "success") {
                        let html = "";
                        response.data.forEach(s => {
                            html += `
                                <tr data-id="${s.id}">
                                    ...
                                    <td>${s.semester}</td>
                                    <td>
                                        <input type="text" value="${s.subject_name}" class="editSubject">
                                    </td>
                                    <td><a href="${s.pdf}" target="_blank">View PDF</a></td>
                                    <td>
            ...
            $("#addSyllabusForm").submit(function (e) {
                e.preventDefault();
                let formData = new FormData(this);
                formData.append("action", "add");

                $.ajax({
                    url: "manage_syllabus.php",
                    type: "POST",
                    data: formData,
                    contentType: false,
                    processData: false,
            ...
            $(document).on("click", ".updateBtn", function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        alert(response.message);
                        if (response.status === "success") fetchSyllabus();
                    }
                });
            });
            ...
            $(document).on("click", ".deleteBtn", function () {
                ...
                if (confirm("Are you sure you want to delete this syllabus?")) {
                    $.post("manage_syllabus.php", { action: "delete", id: id }, function (response) {
                        alert(response.message);
                        if (response.status === "success") fetchSyllabus();
                    }, "json");
                }
